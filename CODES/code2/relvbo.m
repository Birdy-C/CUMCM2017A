...
[p,theta,filter,d,interp,N, cc, d1, d2] = parse_inputs(varargin{:});

[p,H] = filterProjections(p, filter, d, cc, d1, d2);
...
%======================================================================
function [p,H] = filterProjections(p_in, filter, d, cc, d1, d2)

p = p_in;

% Design the filter
len = size(p,1);
H = designFilter(filter, len, d, cc, d1, d2);
...
%======================================================================
function filt = designFilter(filter, len, d, cc, d1, d2)
...
switch filter
    ...
    case 'zhishu'
        filt(2:end) = filt(2:end) .*exp(-cc*w(2:end)./(2*d));%cc可以改成其它值
    case 'zuhe'
        filt(2:end) = filt(2:end) .* (d1*(sin(w(2:end)/(2*d))./(w(2:end)/(2*d))) + d2*(.54 + .46 * cos(w(2:end)/d)));
    otherwise
...
%======================================================================
function [p,theta,filter,d,interp,N, cc, d1, d2] = parse_inputs(varargin)
...
filter_strings = {'ram-lak','shepp-logan','cosine','hamming', 'hann', 'zhishu', 'zuhe', 'none'};
